...
if "vectorstore" not in st.session_state:
    try:
        if os.path.isdir(FAISS_DIR):
            ...
            # For an empty index, provide empty docstore and index_to_docstore_id
            vs = FAISS(embedding_model, index, {}, [])

            # Persist the empty index folder so future loads succeed
        ...
        # Fallback: create empty index again
        dummy_embedding = embedding_model.embed_query(" ")
        dim = len(dummy_embedding)
        index = faiss.IndexFlatL2(dim)
        vs = FAISS(embedding_model, index, {}, [])
